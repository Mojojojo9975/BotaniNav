...
class GeoJsonService {
  ...
  static List<SectionPoint> _parsePoints(Map<String, dynamic> fc) {
    ...
    for (final feature in fc['features'] as List) {
      ...
      // Section_Layer.geojson uses 'Sections' as the label key (e.g. "A-8").
      // SectionConfig.normalise strips dashes and uppercases to match _buildSectionMap keys.
      final rawLabel = (props['Sections'] ?? props['RoomName'] ?? props['name'] ?? '').toString();
      final label = SectionConfig.normalise(rawLabel);
      if (label.isEmpty) continue; // skip null-section features
